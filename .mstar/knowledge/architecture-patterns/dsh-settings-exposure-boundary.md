...
0. **The working fix (n5): official GatewayService RPC channel — bypasses the allowlist entirely.** The web section does NOT depend on the settings exposure allowlist. The plugin registers a `GatewayService` subclass (@deepseek-ai/dsh-type-meta) whose `@Remote` get/set methods become `/api/<namespace>/<method>` endpoints: the host's typertGateway is the SINGLE `/api` RPC interceptor (a plugin must NOT connection.rpc.intercept on /api again — the interceptor slot is single-seat and throws), and its SRC discovery (`ctx.reflect.props` + `remoteMethods` on the typertGateway binding) claims endpoints from ANY registered service, plugin fibers included (same mechanism as the dsh `goals` service). The section calls connection.rpc.call on the /api interceptor with endpoint advisor/get or advisor/set and payload { args } (payload contract: exactly one plain-object `args` field, keys = method parameter names). The in-process write (`ctx.settings.update(ns, patch)` inside the `@Remote` set body) carries **no exposed-namespace check** — the allowlist gate exists only in the apiproxy wire layer, not in the Settings service itself (verified: zero exposed/notExposed references in the settings package). Endpoint form is the SLASH form (advisor/get), not the dot form (advisor.get) — endpoint claims split on `/` and `invokeRpc` requires two slash segments. Multi-fiber dedupe: cordis Service registration fails loud on a duplicate key; catch-and-log so the first fiber owns the gateway.
   - **SRC discovery limitation (2026-08-13, rc.2 dlx host)**: `remoteMethods()` reads a **module-private WeakMap** in `@deepseek-ai/dsh-typert-protocol`. With a locally-linked plugin (`link:` in the profile manifest) under `pnpm dlx -p @deepseek-ai/dsh@0.1.0-rc.2`, the plugin resolves its peers from its real directory (the project's `node_modules` — proven by renaming it: dsh then exits with `Cannot find package ... imported from dsh-advisor/lib/gateway.js`), physically separate from the dlx host tree. The host typertGateway holds its own empty marker table → SRC claims ZERO endpoints → `/api/advisor/*` 404 and the client shows "settings gateway is not ready on this host", while the plugin itself stays Mounted+Enabled (settings.register succeeded; cross-copy schemastery calls were verified healthy — schema double-installation is NOT the breakage). Same-version peers are necessary but not sufficient: module identity, not version, gates the private table.
   - **Robust fix (n5 update): explicit `ctx.typert.register(contribution)` instead of `@Remote` markers.** `TypertRegistry.register` (dsh-typert-registry) writes invocation descriptors into `ctx.typert.local` — the store `claimsEndpoint` and `resolveDescriptor` check **FIRST** (strict path), before any SRC fallback, so claim + dispatch never touch the private table. Contribution shape: `{ package, face: 'host', schemas: [], model: { services: [], events: [], objects: [] }, invocations: [{ id, service: '<serviceKey>', namespace: '<ns>', method, invocation: { kind: 'direct' }, parameters: [{ name, wire, source: 'json', codec: { mode: 'src-json' } }], result: { mode: 'src-json' } }] }` — mirrors the SRC-derived descriptor exactly (direct receiver, src-json codec, JSON wire params). Keep the `TypertRemoteService` base class ONLY for its `typertRemote` binding: dispatch's `validateBinding` requires the visible binding on the live service (a pure instance property). Register through a conditional `ctx.inject(['typert'], ...)` child (registration is a fiber effect) so compositions without a typert registry (headless/standalone/tests) keep the runtime working and simply omit the /api endpoints. Note SRC still passes inside unit tests because tests import one physical copy — the split only appears across physically distinct module instances.
