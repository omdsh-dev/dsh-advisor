...
- **Unit (per module):**
  ...
  - `delivery.ts` — nit injects without waking; concern/blocker steer; immuneTurns downgrade window; advisor-source messages carry the classified first-party `plugin` arm (`kind: 'plugin'`, `plugin: 'advisor'`).
