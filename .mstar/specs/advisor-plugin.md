...
- **S3** Delivery: nit → `agent.inject` (non-waking, next step boundary); concern/blocker → `agent.steer` (waking); `immuneTurns` cooldown (default 3); advisor messages logged on the classified first-party `plugin` source arm (`kind: 'plugin'`, `plugin: 'advisor'`).
...
| delta renderer (role labels, own-message exclusion, primary-context expansion, secret obfuscation) | `session-history-format.ts` + `runtime.ts` `#formatRawDelta` | markdown renderer over `session/event` + `session.events` snapshot (role labels via the surface event stream / `deriveEventMessage`); own-message exclusion via the classified first-party `plugin` source arm (`kind: 'plugin'`, `plugin: 'advisor'` — `isAdvisorMessage`, `src/kinds.ts`); **MVP skips** secret obfuscation and primary-context expansion (see §3.2) |
...
The plugin has two distinct configuration layers: the **persisted global defaults** (§5.1–§5.2 — six keys, one store: the plugin entry config) and the **runtime-only per-session overrides** (§5.3 — in-memory, never persisted). The persisted schema is frozen at exactly the six keys below (§8.6 KD-6); §5.3 adds no persisted key.
...
- `enabled: true` with `provider` **or** `model` missing or empty (the explicit gate requires both) → the advisor **never starts a model call** and reports a disabled-with-reason status (`/advisor status` shows the reason). This is a hard gate, not a warning. **The gate is applied AFTER session resolution (§5.3):** it evaluates the *effective* route — a complete session `modelOverride` pair satisfies the gate for that session even when the global default lacks its pair, while a malformed global schema (unknown key, bad value) still fails closed for every session and cannot be bypassed by any override.
...
### 5.3 Runtime session overrides (not persisted)

Besides the persisted global defaults there is exactly one runtime-only, per-session model override: `modelOverride: { provider, model }` — an **atomic pair** held in memory, keyed by session id. It is never written into the entry config, never persisted, and never replayed from a command log (no schema delta, no sidecar, no custom durable events).

- **Resolution order (one effective resolver):** (1) the global config shape is validated first (malformed still fails closed); (2) enable = session enable override (`/advisor on|off`) ?? global `enabled`; (3) route = the session's complete `modelOverride` pair ?? the composed **global** advisor pair — **half-pairs are never merged** across the two levels; (4) global composition stays user settings → row base → schema defaults. One resolver serves runtime construction, `/advisor status`, and the runtime-affecting signature.
- **Pair validation:** absence inherits; reset deletes. Blank or partial pairs, whitespace-containing identifiers, and extra fields are rejected; outer whitespace is trimmed; case is retained. Setting or resetting a model **never toggles the enable override**; `/advisor off` retains the local pair for a later `/advisor on`. Pinning a pair equal to today's global default still protects the session from a later default edit.
- **Lifetime (KD-5):** the same ephemeral class as the enable override — the pair lives while the same Agent and advisor owner stay live (it survives navigation/reconnection) and is cleared by agent/session disposal, owner teardown, cold resume, or restart. A forked/new session inherits the global defaults. State is O(live overrides): reset removes an emptied entry; no historical SessionId accumulation, no TTL/GC job.
- **Commands (the B1 surface):** `/advisor model` — the effective pair, its source (`session`/`global`), and the live-session lifetime; `/advisor model set <provider> <model>` — the **invoking** session only (separate args allow model ids containing `/`; no arbitrary session-id argument); `/advisor model reset` — re-inherit the current global defaults, never touching the enable override. `/advisor status` reports the effective pair + source; `/advisor config` stays the **global defaults** readback. Validation runs through the app-root LLM service `resolveModelInfo(provider, model, signal)` **before** commit — catalog membership is advisory (manually routable ids allowed) — bound to 60 s (the runtime call-deadline default), cancellable, with **no automatic retries**; failure leaves the previous selection untouched, and a lookup never proves future quota success.
- **Web session control (the B2 surface):** the same controller is exposed to the web client through two plugin-owned gateway endpoints and one session-header action — no second state store, no divergence from the command face.
  - `advisor/getSession({ sessionId })` returns the authoritative session snapshot `{ sessionId, enabled, modelOverride?, modelSource?, effectiveModel?, disabledReason?, lifetime: 'live-session' }` (absent optional keys are omitted on the wire). `advisor/setSessionModel({ sessionId, selection: { provider, model } | null })` returns the post-commit snapshot; `selection: null` = reset (re-inherit the CURRENT global defaults, never touching the enable override). `advisor/get|set` stay **global-only**. Both endpoints are registered through the existing explicit typert contribution — no new first-party dsh methods, no failure-vocabulary expansion: business outcomes ride plugin-domain error tags in the returned data (`advisor/session-unknown` / `advisor/unavailable` / `advisor/rejected` / `advisor/failed` / `advisor/superseded` / `advisor/cancelled`).
  - Unknown or disposed session targets are rejected **without allocating state** (no generation is begun for a dead target). SessionId is **not authorization**: requests still cross the Connection Host/Origin + browser-authentication boundary; there is no raw HTTP endpoint and no session ACL layer. The endpoints are served by the elected reviewer fiber's controller; a gateway fiber without the elected owner answers `advisor/unavailable` and stays inert (no surviving-fiber promotion).
  - The client adds an **Advisor action to `conversation.session.header.actions`** (the session-bound list slot) — NOT the singleton primary-model slot, NOT the root plugin card; the global card (`plugins.bundle.config`) stays global-only. It shows the effective pair, its source (`session`/`global`), and the live-session lifetime; supports set (provider + model) and **Use global default** (reset). Fetch discipline: on menu open, binding change, reconnect and focus/refresh; a client-side binding fence (one controller per session scope binding) plus a per-controller request fence discards late responses for an old binding — a stale response can neither mutate nor render another session's state. The closed action's label is neutral (no model label falsely claiming continuous synchronization); an open menu is a refreshable snapshot; **no push-event contract, no polling**. An unavailable surface never falls back to global writes (`advisor/set`).
...
- **Advisor message shape:** user-role via `createUserMessage({ content, source })`, with `source` on the classified first-party `plugin` arm — `kind: 'plugin'`, `plugin: 'advisor'` (`src/kinds.ts` `ADVISOR_PLUGIN_ID` / `AdvisorSource`) — **pinned** to the one form the advisor declares, `form: 'notice'` with the form-owned bounded `summary` (`AdvisorSource` extracts only that arm, so no other form is expressible). Injected content is self-describing: `[advisor:{severity}] {note}`. The primary system prompt does not mention advisories; the prefix is the only cue how to treat them ("weigh, don't blindly obey" spirit). The plugin-owned identifier belongs in `source.plugin`, **never** in `source.kind` — `kind` is a first-party vocabulary frozen per session-format generation, and a kind the format edge cannot classify makes the whole log untransformable (the V2→V3 classifier refuses it: `cannot safely transform unclassified message source`), leaving the session unopenable.
- **Self-review exclusion:** every message carrying the advisor's classified `plugin` arm (`kind: 'plugin'` + `plugin: 'advisor'`) is excluded from subsequent advisor deltas (the renderer filters it via `isAdvisorMessage`), so the advisor does not read back advice it injected under this shape. **Corollary of the durable rule above:** moving a plugin-owned identifier out of `source.kind` costs the recognizability of notes already persisted under the old identity — a note written before this migration carries the legacy custom kind (`kind: 'advisor'`), is therefore no longer matched by the predicate, and re-enters the advisor delta on any full-surface replay (compaction, a non-append `surfaceOp`, or a delivered-prefix fingerprint mismatch — `src/transcript.ts` `update()` resets and `rebuild()` re-folds the surviving surface from 0, skipping only what the predicate still matches), exactly once per replay. Accepted by design and bounded: no data loss, advisor-side self-review pollution only. No legacy read arm is added deliberately — a compatibility layer that re-matched the legacy kind is forbidden by the project's no-backward-compatibility invariant, which is why this cost is accepted rather than fixed. The price of migrating a source identity, not a defect to be papered over.
...
- **Unit (per module):**
  ...
  - `commands.ts` — session model override (§5.3): atomic-pair parsing accepts and normalizes `set` (outer trim, case retained) and rejects blank pairs, partial pairs, whitespace-containing identifiers, and extra keys; `model reset` and dispose delete the entry (O(live overrides), no SessionId accumulation); model ops never toggle the enable override (`off` retains the local pair); the equal-default explicit pin is distinguishable from inherit in the status source.
  - `index.ts` (wiring) — A/B session isolation (a session's pair affects only its own effective route; other sessions and the global default are untouched, and reset sessions track later global edits); global vs effective readback separation (`/advisor config` = global defaults, `/advisor status` + `/advisor model` = effective pair + source); missing-default-pair vs malformed-global gate distinction (a complete session pair satisfies the former, never the latter); multi-fiber single-owner election (override controller owned by the elected reviewer fiber; non-owners inert) + owner unload cleanup; cold-resume/fork lifetime (the in-memory pair is gone after restart/reload/fork).
  - `gateway.ts` (B2 web surface) — `getSession` snapshot shape pins `lifetime: 'live-session'` (with `modelOverride`/`modelSource`/`effectiveModel` present iff a pin/effective pair exists and `disabledReason` carried when the gate blocks); set/reset through the endpoints equal the command-face semantics (one controller — the same validation, fencing, and route-change effects); selection validation shares the command face's pair rules; unknown/disposed targets answer `advisor/session-unknown` in data with NO validation lookup started (no generation allocated); a validation failure maps to `advisor/failed` with the previous state untouched; the global entry config is untouched by the session surface; a gateway without the elected owner answers `advisor/unavailable`.
  - client (B2 session action) — every session call carries its own binding's `sessionId` and commits only into its own controller (a late response for one binding never renders into another); a stale response superseded by a newer op is discarded wholesale (request fence); an unavailable latch (unavailable/unknown tag or transport failure) refuses writes with no rpc call and never falls back to the global `advisor/set` channel; the closed action label is neutral; open fetches, and a refresh-signal bump (reconnect/focus) refetches an open menu idempotently per epoch; reset with a missing global pair renders the gate-blocked/no-call state from the host's snapshot without a further call.
  - `advisor-runtime.ts` (route change) — an effective-pair change aborts the session's old advisor call, drops the backlog, re-seeds to the current session seq, and takes effect at the next reviewable delta (no replay of the current window on route change); the same effective pair never restarts a runtime; a late note from the old route is never delivered after the change; validation lookups (`resolveModelInfo`, 60 s deadline, cancellable, no auto-retry) are fenced by per-session generation — a newer set/reset supersedes unresolved older work, and dispose/teardown cannot be undone by a delayed validation completion.
...
| Typecheck | `pnpm exec tsc --noEmit` (strict, `moduleResolution: bundler`) | exit 0 — probe exercising `session/event` + `findLastMessageTurnEnd`, `ctx.llm.stream` with `GenerateOptions`, `createUserMessage` with the classified first-party `plugin` source arm (`kind: 'plugin'`, `plugin: 'advisor'`), `ctx.agents.get`, command registration — all type-clean, augmentations applied |
| Runtime | `node --input-type=module` importing `@deepseek-ai/dsh-llm`, `@deepseek-ai/dsh-session`, `cordis` | exit 0 — `createUserMessage` with `{ kind: 'plugin', plugin: 'advisor' }` accepted, `findLastMessageTurnEnd` callable |
...
- **Output-token cap:** the advisor call sets `maxTokens: 256` so a runaway reply cannot blow the budget; the frame must fit within it. **[Superseded (2026-08-11, n4 user direction — qc2 S-2 / qc1 S-2 / qc3 F-2):** the advisor call runs with `ADVISOR_MAX_TOKENS = 5120` (256 → 5120, 20x) so even a reasoning-heavy reply cannot starve the JSON frame — see KD-6 (§8.6). The historical 256 text above is preserved for context; the raised ceiling is re-bounded downstream (`extractAdviceNote` `ADVISOR_NOTE_MAX_CHARS` cap, bounded notice summary).] **[Superseded again (2026-08-16, user direction):** with `reasoningEffort: 'off'` the capability-gated default, the 20× reasoning headroom is unnecessary — `ADVISOR_MAX_TOKENS = 768` (5120 → 768), a modest budget that fits one `ADVISOR_NOTE_MAX_CHARS` note (768 chars) plus the JSON frame; see KD-6 (§8.6).]**
...
- **Session model override lifecycle (§5.3, B1):** the runtime `modelOverride` pair rides the same lifetime class as the enable override. **Reset-to-inherit:** `/advisor model reset` deletes the session's pair, so the route re-inherits the *current* global defaults — later global edits reach a reset session immediately, a pinned session never; reset never touches the enable override, and an entry emptied by reset (or by enable-off with nothing else held) is removed so state stays O(live overrides). **Cold resume:** a session restored after a process restart, advisor-owner reload, or cold (non-live) resume starts with the global defaults — the in-memory pair is gone and nothing replays it (no command-log replay, no durable events); live navigation/reconnection within the same Agent and owner keeps it. A forked/new session inherits the global defaults.

### 8.6 KD-6 — minimal advisor request contract (zero tools, capability-gated thinking-off) — RESOLVED

**Decision: every advisor `ctx.llm.stream` call is a minimal start — the closed AC-1 `GenerateOptions` whitelist, zero `tools` key, capability-gated `reasoningEffort: 'off'`, `purpose` unset (KD-5), and `maxTokens` 768 (KD-2 supersession chain: 256 → 5120 → 768). The guarantee is a code invariant, not an operator switch.**
...
- **No new plugin config keys:** the single §5.1 schema surface is unchanged (`enabled` / `provider` / `model` / `systemPrompt` / `immuneTurns` / `maxDeltaMessages`); no `thinking` / `tools` / `reasoningEffort` / "minimal start" toggle keys exist. The freeze governs the **persisted** schema: the per-session model override (§5.3) is a **runtime-only command surface** (`/advisor model set|reset`) with **no persisted delta** — the six persisted keys stay the only persisted keys, and the override never appears in the entry config. The minimal-request whitelist, capability-gated thinking-off, `purpose` unset (KD-5), and the 768-token cap are untouched by the override: a session-pinned route is subject to the exact same request contract as the global route.
- **`purpose` unset (KD-5); `maxTokens` 768 (KD-2 supersession chain):** an advisor call is an ordinary conversation request and leaves `purpose` unset (closed union `'compaction' | 'session-title'`). `maxTokens` is `ADVISOR_MAX_TOKENS` (768) — the user-directed supersession of the 5120 value (itself the supersession of KD-2's historical `256`, §8.2 annotations above): with thinking-off the default, a modest budget that fits one `ADVISOR_NOTE_MAX_CHARS` note (768 chars) plus the JSON frame; the ceiling is re-bounded downstream (`extractAdviceNote` `ADVISOR_NOTE_MAX_CHARS` cap, bounded notice summary).
