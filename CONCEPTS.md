...
私有 `@deepseek-ai/*` 包**只**作 `peerDependencies`（绝不进 `dependencies` / `devDependencies`）：`pnpm-workspace.yaml` 的 `autoInstallPeers: true` + 用户级 `~/.npmrc` registry 认证令牌从 npm registry 解析 `@deepseek-ai/*`（pnpm 11 起项目级 `.npmrc` 不再展开 `${NPM_TOKEN}`；无本地 link farm，`nodeLinker: hoisted`）。`tests/peer-deps.test.ts` 数据驱动钉住该契约（peer-only、`dsh-*` peers 共享 package.json 中的同一 range、autoInstallPeers、scoped schemastery、prepare 仅 build）。运行时值 import 保持 external，由宿主 in-box / 扁平 profile 模块 fallback 解析。
