...
| `gateway.test.ts` | 22 | 无 settings service（entry fallback，`get` 可用，gateway 是已注册服务）；有 settings service（`set` 写 entry config、值 live 变化、`set` 返回新合成值、merge 语义、空/null patch 为 no-op）；`set` 校验（未知键在写入前被拒）；硬门禁回归（`resolveAdvisorConfig` 保持 SSOT）；typertGateway 端点认领 + payload 契约（`{ args }` 恰好一个 plain-object 字段、`/api` interceptor 分发、直接经 `ctx.typertGateway` 调用、bad entry config 的 get containment）；组合插件（apply wiring 后 dispatch 对 live 合成配置生效、多 fiber 去重） |
...
| `peer-deps.test.ts` | 6 | registry peer 契约：每个 `@deepseek-ai/*` 都是 peerDependency 且不出现在其它 dependency 字段、`dsh-*` peers 共享 package.json 中的同一 range、无 link farm（从 npm 解析） |
...
4. 选择 provider/model 并保存 → **预期**：保存成功（经 `/api/advisor/set` → `settings.update('advisor', …)` → config editor → Loader）；该 profile 补丁层（`~/.dsh/profiles/<profile>/cordis.patch.yml`）的 `advisor` 行 `config` 更新为含 `enabled: true` 与所选 provider/model；重进页面显示已保存值。
...
2. `/advisor on` / `/advisor off` / 裸 `/advisor`（toggle）→ **预期**：翻转会话级状态、回复带 S4 caveat（当门禁阻挡）；持久化配置**不变**（profile 补丁层的 advisor 行 config 无改动——指令是临时 override，不写持久化配置）。
...
| 写入真实 `$DSH_HOME` / profile 补丁层（advisor 行 config） | 沙箱禁止对运行中 dsh 安装写入 | 用户 §1/§2/§5（settings/gateway 写入逻辑已由 `settings.test.ts` / `gateway.test.ts` / `settings-live.test.ts` 覆盖） |
