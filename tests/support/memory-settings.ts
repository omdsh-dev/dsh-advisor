 ...
 * `Settings` base class from `@deepseek-ai/dsh-settings` — the base owns
 * namespace registration, composition resolution, validation, revisions,
 * watchers, and the `settings/updated` commit event; only raw-document
 * storage is abstract. This is the dev-time stand-in for the file-backed
 * `@deepseek-ai/dsh-settings-local` (which the advisor tests would otherwise
 * have to drive through temp YAML documents), so the tests exercise the real
